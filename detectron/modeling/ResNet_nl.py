# ====================================================================
# Northrend@github.com 2018-04-20 15:09:48
# Modified from official Detectron code:
# https://github.com/Northrend/Detectron.git
# ====================================================================
#
...
from core.config import cfg    # import nonlocal configuration

from modeling import non_local as non_local_op
...
# Add non-local block, set non_local=[0,0,0,1,0] to bolt onto res4
def add_ResNet101_nl_conv5_body(model):
    return add_ResNet_convX_body(model, (3, 4, 23, 3), non_local=[0,0,0,1,0])
    ...
    if nl_last_block:
        ...
        for i in range(n):
            blob_in = add_residual_block(
                ...
                inplace_sum=i < n - 1,
                non_local=False if i<(n-1) else True
            )
            dim_in = dim_out
...
def add_ResNet_convX_body(model, block_counts, freeze_at=2, non_local=[0 for i in range(5)]):
    ...
    s, dim_in = add_stage(model, 'res2', p, n1, dim_in, 256, dim_bottleneck, 1, nl_last_block=non_local[1])
    if freeze_at == 2:
        model.StopGradient(s, s)
    s, dim_in = add_stage(
        model, 'res3', s, n2, dim_in, 512, dim_bottleneck * 2, 1, nl_last_block=non_local[0]
    )
    if freeze_at == 3:
        model.StopGradient(s, s)
    s, dim_in = add_stage(
        model, 'res4', s, n3, dim_in, 1024, dim_bottleneck * 4, 1, nl_last_block=non_local[3]
    )
    if freeze_at == 4:
        model.StopGradient(s, s)
    if len(block_counts) == 4:
        ...
        s, dim_in = add_stage(
            ...
            cfg.RESNETS.RES5_DILATION, nl_last_block=non_local[4]
        )
        if freeze_at == 5:
            model.StopGradient(s, s)
...
def add_residual_block(
    ...
    # ---- non-local ----
    if non_local:
        nl = non_local_op.add_nonlocal(
        model, 
        blob_in, 
        dim_in, 
        dim_in,    # dim_out 
        cfg.NONLOCAL.BATCHSIZE,    # batch_size 
        prefix, 
    ...
    # transformation blob
    tr = globals()[cfg.RESNETS.TRANS_FUNC](    # __C.RESNETS.TRANS_FUNC = b'bottleneck_transformation'
        model,
        blob_in,
        dim_in,
        dim_out,
...
def test():
    ...
    from caffe2.python import cnn, net_drawer, workspace, core
    ...
    # model = cnn.CNNModelHelper(name='resnet-50')
    model = DetectionModelHelper(
            name='test_generalized_rcnn',
            train=True,
            num_classes=2,
            init_params=True)
    # model.net.AffineChannel()
    merge_cfg_from_file(sys.argv[1])
    # add_ResNet101_conv5_body(model)    # proto resnet-101
    add_ResNet101_nl_conv5_body(model)    # resnet-101 with non-local
    ...
    # net = workspace.Predictor(model, model)
    
    tmp = net_drawer.GetPydotGraph(model, rankdir="TB")
    # tmp = net_drawer.GetPydotGraphMinimal(model, rankdir="TB")
    tmp.write_dot('/workspace/northrend/tmp_viz/'+ model.Proto().name + '.dot')
